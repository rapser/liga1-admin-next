...
export class PushNotificationService {
  ...
  > {
    ...
    if (successCount === 0 && topics.length > 0) {
      throw new Error(
        `No se pudo enviar la notificación a ningún topic. Errores: ${results.map((r) => r.error).join(", ")}`,
      );
    }
  ...
  async sendGoalNotification(
    ...
    if (topics.length === 0) {
      throw new Error(
        `No se encontraron topics válidos para los equipos del partido. Match ID: ${match.id}`,
      );
    }

    const teamName = getTeamFullName(teamCode);
    const localName = normalizedMatch.equipoLocalId
      ? getTeamFullName(normalizedMatch.equipoLocalId)
      : "Local";
    const visitorName = normalizedMatch.equipoVisitanteId
      ? getTeamFullName(normalizedMatch.equipoVisitanteId)
      : "Visitante";
  ...
  async sendMatchEndNotification(match: Match): Promise<void> {
    ...
    for (const topic of topics) {
      ...
      const isLocalTeam =
        normalizedMatch.equipoLocalId &&
        topic === getTeamTopic(normalizedMatch.equipoLocalId);
      const teamId = isLocalTeam
        ? normalizedMatch.equipoLocalId!
        : normalizedMatch.equipoVisitanteId!;
      ...
      if (isLocalTeam) {
        if (
          normalizedMatch.golesEquipoLocal >
          normalizedMatch.golesEquipoVisitante
        ) {
          resultText = "¡Victoria!";
          result = "win";
        } else if (
  ...
  async sendSilentNotification(
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Error al enviar la notificación silenciosa",
        );
      }
  ...
  async sendScoreUpdateNotification(
    ...
    if (!equipoLocalId || !equipoVisitanteId) {
      throw new Error(
        `No se pudieron determinar los códigos de equipos del partido. Match ID: ${match.id}`,
      );
    }
